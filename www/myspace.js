...
exports.MySpace = (function(global) {
	var K = function(){};
	
	var MySpace = function(options) {
		var self;
    
	    if (this instanceof MySpace) {
	      self = this;
	    } else {
	      self = new K();
	    }
	...
	K.prototype = MySpace.prototype;
	
	MySpace.prototype.onLocationChange = function(loc) {
		var self = this, oauth = this.oauthClient;
		console.log("MySpaceLog: onLocationChange : " + loc);
	   ...
	   // The supplied oauth_callback_url for this session is being loaded
	    if (loc.indexOf(self.callbackUrl+"?") >= 0) {
            ...
            oauth.get('http://api.myspace.com/access_token?oauth_verifier='+verifier+'&'+self.requestParams,
                    function(data) {               
                        var accessParams = {};
                        ...
                        console.log('MySpaceLog: ' + accessParams.oauth_token + ' : ' + accessParams.oauth_token_secret);
                        ...
                        console.log("MySpaceLog: Storing token key/secret in localStorage");
                ...
                function(data) { 
                    self.authorized = false;
                    console.log("MySpaceLog: 1 Error " + data.text); 
		            self.fireEvent('login', {
		              success: false,
		              error: data.text,
		              result: data
	...
	*/
	MySpace.prototype.authorize = function() {
		var self = this, oauth = this.oauthClient;
		...
		if (this.authorized) {
		  ...
		  setTimeout(function() {
		    ...
		    oauth.get('http://api.myspace.com/request_token',
			    function(data) {
			        ...
			        console.log("MySpaceLog: requestParams: " + data.text);
			        self.webView.showWebPage('http://api.myspace.com/authorize?'+data.text, 
			                { showLocationBar : false });  
			        self.webView.onLocationChange = function(loc){self.onLocationChange(loc);};	                 
			    },
			    function(data) { 
			        console.log("MySpaceLog: 2 Error " + data.text); 
			    }
		    );
   ...
   /* Make an authenticated MySpace API request.
   * 
   * @param {String} path the MySpace API path without leading forward slash. For example: `v2/user/info`
   * @param {Object} params  the parameters to send along with the API call
   * @param {String} [httpVerb="GET"] the HTTP verb to use
   * @param {Function} callback
   */
	MySpace.prototype.request = function(path, params, headers, httpVerb, callback) {
		var self = this, oauth = this.oauthClient, url;
		
		if (path.match(/^https?:\/\/.+/i)) {
		    url = path;
		} else {
		    url = 'http://api.myspace.com/' + path;
		}
	...
	MySpace.prototype.logout = function(callback){
		var self = this;
	...
	*/
	MySpace.prototype.addEventListener = function(eventName, callback) {
	...
	* Fire an event
	*/
	MySpace.prototype.fireEvent = function(eventName, data) {
